...
sample_dist=c("sregular", "wregular", "random",
              "wclust","sclust")
train_points <- sim_samples(100, sample_dist[5], spoly)
...
for (i in 1:NCV){
  ...
  ntest = nrow(testDatB)
  modB = ranger(response ~., data = trainDatB, mtry = 2, quantreg = TRUE)
...
boxplot(cbind(resu[[1]]$q2,resu[[2]]$q2))
boxplot(cbind(resu[[1]]$ES,resu[[2]]$ES))
boxplot(cbind(resu[[1]]$VS,resu[[2]]$VS))
boxplot(cbind(resu[[1]]$Coverage,resu[[2]]$Coverage))
